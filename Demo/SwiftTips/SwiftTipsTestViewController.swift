...
class SwiftTipsTestViewController: UIViewController {
    ...
    let titlesArray = ["初始化方法顺序", "Static和Class", "多类型和容器", "Lazy", "异常", "Codable"]
...
//        logIfTrue { 2 > 1 }
    ...
    func setupUI() {
        ...
        tableView.register(UITableViewCell.self, forCellReuseIdentifier: "TipsCellID")
        view.addSubview(tableView)
    }
}

extension SwiftTipsTestViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return titlesArray.count
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "TipsCellID")
        cell?.textLabel?.text = "\(indexPath.row + 1): \(titlesArray[indexPath.row])"
        return cell!
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 0 {
            let vc = InitTestViewController()
            self.navigationController?.pushViewController(vc, animated: true)
        }else if indexPath.row == 1 {
            let vc = StaticAndClassViewController()
            self.navigationController?.pushViewController(vc, animated: true)
        }else if indexPath.row == 2 {
            let vc = SwiftTipsTest3()
            self.navigationController?.pushViewController(vc, animated: true)
        }else if indexPath.row == 3 {
            let vc = SwiftTipsTest4()
            self.navigationController?.pushViewController(vc, animated: true)
        }else if indexPath.row == 4 {
            let vc = SwiftTipsTest5()
            self.navigationController?.pushViewController(vc, animated: true)
        }else if indexPath.row == 5 {
            let vc = SwiftTipsTest6()
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
}
